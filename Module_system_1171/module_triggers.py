...
# Tutorial:
  ...
  (0.5, 0, ti_once, [(map_free,0),(this_or_next|eq,"$background_answer_4",cb4_brother),(eq,"$background_answer_4",cb4_sister),], [
        (try_begin),
          (eq,"$background_answer_4",cb4_brother),
          (assign,":cur_npc","trp_brother"),
          (str_store_string,s2,"str_start_quest_find_brother"),
          (call_script, "script_start_quest", "qst_find_brother",0),
        (else_try),
          (eq,"$background_answer_4",cb4_sister),
          (assign,":cur_npc","trp_sister"),
          (str_store_string,s2,"str_start_quest_find_sister"),
          (call_script, "script_start_quest", "qst_find_sister",0),
        (try_end),
        (assign,"$troop_need_rename",":cur_npc"),
        (start_presentation,"prsnt_name_npc"),
        # (party_add_members, "p_main_party",":cur_npc", 1)
        ]),
...
#    ]),
  ...
  [
      ...
      (party_slot_eq, ":center_no", slot_center_has_manhunter_union, 1),
      (spawn_around_party, ":center_no", "pt_manhunters"),
  ]),

  (0, 0, ti_once, #new#init proficiency
  [],
  [
    (assign,":fire_level",150),
    (try_for_range,":cur_troop",soldiers_begin,"trp_quick_battle_6_player"),
        (store_proficiency_level,":fire_level",":cur_troop",wpt_crossbow),

      (troop_raise_proficiency_linear,":cur_troop",wpt_firearm,":fire_level"),
    (try_end),
  ]),

  (24, 0, 0.0, 
  [
    (store_num_parties_of_template, reg2, "pt_manhunt_knight"),    
    (lt, reg2, 3)
  ],
  [
    (set_spawn_radius, 1),
    ...
    (spawn_around_party, ":selected_town", "pt_manhunt_knight"),
  ]),

  (24, 0, 0.0, 
  [
    (store_num_parties_of_template, reg2, "pt_outlaw_knight"),    
    (lt, reg2, 3)
  ],
  [
    (set_spawn_radius, 1),
    (store_add, ":p_town_22_plus_one", "p_town_22", 1),
    (store_random_in_range, ":selected_town", "p_town_1", ":p_town_22_plus_one"),
    (spawn_around_party, ":selected_town", "pt_outlaw_knight"),
  ]),
