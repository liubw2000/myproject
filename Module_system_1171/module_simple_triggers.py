...
#Notification menus
  ...
  #Music,
  (1,
   [   (call_script,"script_init_magic_stuff"),
       (map_free),
   ...
   (8, #increased from 12
    ...
    (0.5,	
     [
        ...
        #new libwe
        (store_faction_of_troop, ":O_faction", ":troop_no"),
        (neg|faction_slot_eq,":O_faction",slot_faction_state,sfs_inactive),
...
##    ]),
  ...
  (14,
   [
    ...
    (assign,":undead_num",0),#libwe undead did not cost food
    (try_for_range,":cur_soilder","trp_undead_walker","trp_undead_end"),
      (party_count_members_of_type, ":cur_troop_num","p_main_party",":cur_soilder"),
      (val_add,":undead_num",":cur_troop_num"),
    (try_end),
    (val_sub,":num_stacks",":undead_num"),
    ...
    (try_begin),#new#libwe
      (assign,":continue",0),
      (try_for_range,":npc_no","trp_npc1","trp_kingdom_1_lord"),
        (eq,"$g_player_quartermaster",":npc_no"),
        (main_party_has_troop,":npc_no"),
        (assign,":continue",1),
      (try_end),
      (eq,":continue",1),
      (store_mul,":money_cost",":consumption_amount",10),
      (val_div,":money_cost",50),
      (assign,reg1,":money_cost"),
      (display_message,"@cosume_money{reg1}"),
      (troop_remove_gold,"trp_player",":money_cost"),
    (else_try),
 ...
 #Update how good a target player is for bandits
  ...
  (72,
   [
     ...
     (call_script, "script_update_mercenary_intermediary"),#libwe
     ...
     (try_end),
     (try_for_range, ":troop_no",  "trp_constable_hareck", "trp_deliver"),
        (troop_get_slot,":merchant_num",":troop_no",slot_troop_merchant_num),
        (le,":merchant_num",50),
        (val_add,":merchant_num",3),
        (troop_set_slot,":troop_no",slot_troop_merchant_num,":merchant_num"),
     (try_end),
    ]),

    (24*3,#stability corruption
   [
    (try_for_range,":town_no",centers_begin,centers_end),
      (party_get_slot,":cur_stability",":town_no", slot_center_stability),
      (party_get_slot,":cur_corruption",":town_no", slot_chaos_corruption),

      (try_begin),
        (eq,":cur_stability",0),#1 stability #2 corruption
        (assign,":change1",0),
      (else_try),
        (gt,":cur_stability",0),
        (assign,":change1",-1),
      ...
      (party_get_slot, ":center_lord", ":town_no", slot_town_lord),#decided by lord reputation type
      (try_begin),
        (gt,":center_lord",0),
        (val_sub,":change1",1),
        (val_sub,":change2",1),
        (troop_get_slot,":lord_reputation_type",":center_lord",slot_lord_reputation_type),
        (try_begin),
          (this_or_next|eq,":lord_reputation_type",lrep_upstanding),
          (this_or_next|eq,":lord_reputation_type",lrep_cunning),
          (eq,":lord_reputation_type",lrep_martial),
          (val_add,":change1",6),
          (val_add,":change2",6),
        (else_try),
          (this_or_next|eq,":lord_reputation_type",lrep_goodnatured),
          (this_or_next|eq,":lord_reputation_type",lrep_selfrighteous),
          (eq,":lord_reputation_type",lrep_benefactor),
          (val_add,":change1",3),
          (val_add,":change2",3),
        (try_end),
      (else_try),
      ...
      (val_add,":cur_stability",":change1"),
      (val_add,":cur_corruption",":change2"),

      (party_set_slot,":town_no", slot_center_stability,":cur_stability"),
      (party_set_slot,":town_no", slot_chaos_corruption,":cur_corruption"),
    (try_end),
    ]),
   ...
   [
      (try_begin),
        ...
        (eq,":ran",0),
        (store_random_in_range,"$magic_wind",1,101),
        (try_begin),
          (store_skill_level,":magic_power","skl_reserved_5","trp_player"),
          (gt,":magic_power",5),
          (display_message,"@magicwindc",0x3399FF),
        (try_end),
      (try_end),
    ]),

    #libwe#
      (24*7,#update_nobleman_troops
   [
      (try_begin),
        (eq,"$cheat_mode",1),
        (display_message,"@update nobleman"),
      (try_end),

     (try_for_range, ":town_no", walled_centers_begin, walled_centers_end),
       (call_script, "script_update_nobleman_troops", ":town_no"),
     (try_end),
    
    (try_begin),
      (store_random_in_range,":ran",0,2),
      (eq,":ran",1),
      (call_script, "script_try_spawn_random_hero_party"),
    (try_end),

     (try_for_range, ":troop_no",  "trp_kingdom_heroes_including_player_begin", "trp_knight_1_1_wife"),
        (troop_slot_ge,":troop_no",slot_lord_personal_troop,1),
        (store_random_in_range,":rand",0,8),
        (troop_set_slot,":troop_no",slot_troop_merchant_num,":rand"),
     (try_end),
    ]),

    (24,#lord specific skill#
   [
        (try_for_range,":troop_id", active_npcs_begin, active_npcs_end),
          ...
          (try_for_range,":dest",0,3),
            ...
            (troop_get_slot,":specific_skill",":troop_id",":slot_no"),
            (try_begin),
              (eq,":specific_skill","str_lord_1"),
              (troop_get_slot, ":initial_wealth", ":troop_id", slot_troop_wealth),
              (val_add,":initial_wealth",200),
              (troop_set_slot, ":troop_id", slot_troop_wealth, ":initial_wealth"),
            (else_try),
              (eq,":specific_skill","str_lord_3"),
              (troop_get_slot,":hero_party",":troop_id",slot_troop_leaded_party),
              (gt,":hero_party",0),
          ...
          (try_begin),
            (troop_get_slot, ":lord_party", ":troop_id", slot_troop_leaded_party),
            (ge, ":lord_party", 0),
            (party_get_attached_to, ":lord_attachment", ":lord_party"),
            (is_between, ":lord_attachment", centers_begin, centers_end),#in a center
            (party_get_slot,":cur_stability",":lord_attachment", slot_center_stability),
            (party_get_slot,":cur_corruption",":lord_attachment", slot_chaos_corruption),
            (val_add,":cur_stability",":change1"),
            (val_add,":cur_corruption",":change2"),
            (party_set_slot,":lord_attachment", slot_center_stability,":cur_stability"),
            (party_set_slot,":lord_attachment", slot_chaos_corruption,":cur_corruption"),
          (try_end),
        (try_end),
    ]),
   ...
   [
    ...
    (try_for_range,":unused",0,5),
      ...
      (try_for_range,":des",0,6),
        ...
        (party_get_slot,":have_mafia",":town_no",":slot_no"),
        (try_begin),
          (eq,":have_mafia",1),
          (assign,":continue",0),
        (try_end),
      ...
      (eq,":continue",1),
      (store_add,":slot_add_1",slot_center_has_mafia6,1),
      (store_random_in_range,":slot_no",slot_center_has_mafia1,":slot_add_1"),
      (party_set_slot,":town_no",":slot_no",1),
    (try_end),
    ]),
   ...
   [
    (try_for_range,":town_no",towns_begin,towns_end),
      (try_for_range,":des",0,6),
        ...
        (try_begin),
          ...
          (try_begin),
            (ge,":val",8),
            (neg|party_slot_eq,":town_no",slot_town_lord,"trp_player"),
            (party_set_slot,":town_no",":slot_no",0),#destory mafia,2/10
          (else_try),      
            (call_script, "script_change_center_prosperity",":town_no", -1),
            (party_get_slot,":cur_stability",":town_no", slot_center_stability),#stability
            (val_sub,":cur_stability",3),
            (party_set_slot,":town_no", slot_center_stability,":cur_stability"),
            (le,":val",5),
            (store_add,":pt_no",":des","pt_mafia1"),
            (spawn_around_party, ":town_no",":pt_no"),
            (assign,":new_party",reg0),
            (call_script,"script_sort_party_by_troop_level",":new_party"),
          (try_end),
        (try_end),
      (try_end),
    (try_end),
    ...
    [  #update_repair_time 
  (try_begin),
     (try_for_range,":troop","trp_town_1_armorer","trp_town_1_mayor"),
     (troop_slot_ge,":troop",slot_troop_hold_items_time,1),
    # (call_script, "script_update_item_repaired_time", ":troop"),
     (troop_get_slot,":repair_time",":troop",slot_troop_hold_items_time),
     (val_sub,":repair_time",1),
     (troop_set_slot,":troop",slot_troop_hold_items_time,":repair_time"),
  (try_end),
  (try_begin),
     (try_for_range,":troop","trp_town_1_armorer","trp_town_1_mayor"),
     (neq, "$g_player_is_captive", 1),
     (troop_slot_eq,":troop",slot_troop_hold_items_time,0),
     (troop_slot_ge,":troop",slot_troop_hold_items,1),
     (assign,"$troop_finish_repair",":troop"),
     (store_random_in_range,":dna",0,1000),
     (start_map_conversation,"trp_deliver",":dna"),
  (try_end),
    ]),
    ...
    (5,
   [
      (check_quest_active,"qst_destory_bandit_zendar"),
      (set_spawn_radius, 5),
      (spawn_around_party,"p_zendar","pt_looters"),
      (spawn_around_party,"p_zendar","pt_looters"),
      (spawn_around_party,"p_zendar","pt_looters"),
    ]),
    (0,
   [
      (check_quest_active,"qst_destory_bandit_zendar"),
      (store_num_parties_destroyed_by_player,":cur_num","pt_looters"),
      (val_sub,":cur_num","$looters_de_num_begin"),
      (store_sub,":num",5,":cur_num"),
      (try_begin),
        (le,":num",0),
        (call_script,"script_succeed_quest"),
      (else_try),
        (quest_get_slot, ":quest_giver_troop", "qst_destory_bandit_zendar", slot_quest_giver_troop),
        (assign,reg1,":num"),
        (str_store_troop_name, s9, ":quest_giver_troop"),
        (str_store_string,s2,"@huntlootersupdate"),
        (add_quest_note_from_sreg, "qst_destory_bandit_zendar", 2, s2, 0),
      (try_end),
    ]),
    (0,#sea_raider
   [
      (check_quest_active,"qst_destory_sea_raider_zendar"),
      (store_num_parties_destroyed_by_player,":cur_num","pt_sea_raiders"),
      (val_sub,":cur_num","$looters_de_num_begin"),
      (store_sub,":num",5,":cur_num"),
      (try_begin),
        (le,":num",0),
        (call_script,"script_succeed_quest"),
      (else_try),
        (quest_get_slot, ":quest_giver_troop", "qst_destory_sea_raider_zendar", slot_quest_giver_troop),
        (assign,reg1,":num"),
        (str_store_troop_name, s9, ":quest_giver_troop"),
        (str_store_string,s2,"@huntlootersupdate1"),
        (add_quest_note_from_sreg, "qst_destory_sea_raider_zendar", 2, s2, 0),
      (try_end),
    ]),

  (24*3,#spawn looter group
  [ 
    ...
    (spawn_around_party, ":selected_town", "pt_lootergroup"),
    (assign,":new_party",reg0),
    (call_script,"script_sort_party_by_troop_level",":new_party"),
  ]),
  (24*3,#spawn femaleknight
  [ 
    (store_random_in_range,":des",0,10),
    (gt,":des",5),
    (set_spawn_radius, 1),
    (store_add, ":p_town_22_plus_one", "p_town_22", 1),
    (store_random_in_range, ":selected_town", "p_town_1", ":p_town_22_plus_one"),
    (spawn_around_party, ":selected_town", "pt_woman_knight"),
    (assign,":new_party",reg0),
    (call_script,"script_sort_party_by_troop_level",":new_party"),
  ]),
  (24*2,#spawn wabber group
  [ 
    (store_random_in_range,":des",0,10),
    (gt,":des",3),
    (set_spawn_radius, 2),
    (store_random_in_range,":selected_town","p_town_7","p_town_10"),
    (spawn_around_party, ":selected_town", "pt_wabber_group"),
    (assign,":new_party",reg0),
    (call_script,"script_sort_party_by_troop_level",":new_party"),
  ]),
  (30,#spawn brotherhood or hunter
  [ 
    (store_random_in_range,":des",0,10),
    (try_begin),
      (gt,":des",4),
      (set_spawn_radius,5),
      (spawn_around_party, "p_town_4", "pt_brotherhood"),
    (else_try),
      (set_spawn_radius,2),
      (spawn_around_party, "p_town_4", "pt_brotherhood"),
      (assign, ":new_party", reg0),
      (party_set_ai_behavior,":new_party",ai_bhvr_patrol_party),
      (party_set_ai_object,":new_party","p_town_4"),
      (party_set_ai_patrol_radius,":new_party",25),
    (try_end),
    (assign,":new_party",reg0),
    (call_script,"script_sort_party_by_troop_level",":new_party"),
  ]),
  (24,#spawn nord valkyrie or hunter
  [ 
    (store_random_in_range,":des",0,12),
    (set_spawn_radius, 10),
    (store_random_in_range,":selected_town","p_sea_raider_spawn_point_1","p_sea_raider_spawn_point_2"),
    (try_begin),
      (gt,":des",7),
      (assign,":spr_party","pt_nord_valkyrie"),
    (else_try),
      (assign,":spr_party","pt_nord_valkyrie_hunter"),
    (try_end),
    (spawn_around_party, ":selected_town", ":spr_party"),
    (assign,":new_party",reg0),
    (call_script,"script_sort_party_by_troop_level",":new_party"),
  ]),
  (24*7,#spring knight
  [ 
    ...
    (store_add, ":p_town_22_plus_one", "p_town_22", 1),
    (store_random_in_range, ":selected_town", "p_town_1", ":p_town_22_plus_one"),
    (spawn_around_party, ":selected_town", "pt_spring_knight"),
    (assign,":new_party",reg0),
    (call_script,"script_sort_party_by_troop_level",":new_party"),
  ]),
  (24*2,#spring guard
  [ 
    ...
    (gt,":des",5),
    (set_spawn_radius, 3),
    (spawn_around_party, "p_spring_knight_castle", "pt_spring_lake_guard"),
    (assign,":new_party",reg0),
    (party_set_ai_behavior,":new_party",ai_bhvr_patrol_party),
    (party_set_ai_object,":new_party","p_spring_knight_castle"),
  ...
  [ 
    ...
    (spawn_around_party, ":selected_town", "pt_gulfod_army"),
    (assign,":new_party",reg0),
    (call_script,"script_sort_party_by_troop_level",":new_party"),
  ]),
  (24*7,#askr_army
  [ 
    ...
    (set_spawn_radius, 25),
    (store_random_in_range, ":selected_town",towns_begin,towns_end),
    (spawn_around_party, ":selected_town", "pt_askr_army"),
    (assign,":new_party",reg0),
    (call_script,"script_sort_party_by_troop_level",":new_party"),
  ]),
  #random sprawn
  ...
  (50,#adventurer group
  [
    (call_script,"script_spawn_adventurer_group"),  
  ]),
  #hero party spawn
  (24*7,
  [
    ...
    (24,#add personal troop
  [ 
    (call_script,"script_add_personal_troop_to_npc"),
  ]),
    (24*7,#hire guard party
  [ 
    (call_script, "script_try_hire_guard_party"),
  ]),
    (24*3,#change center culture
  [ 
    (try_for_range,":cur_center",centers_begin,centers_end),
      (store_faction_of_party, ":center_faction", ":cur_center"),
      (faction_get_slot,":new_culture",":center_faction",slot_faction_culture),
      (party_set_slot,":cur_center",slot_center_culture,":new_culture"),
    (try_end),
  ]),
  ...
  (12,
   [(try_for_range, ":center_no", centers_begin, centers_end),
      ...
      (else_try),
        (is_between, ":center_no", towns_begin, towns_end),#new #libwe
   ...
   [(assign, ":num_active_tournaments", 0),
    ...
    (try_for_range, ":center_no", centers_begin, centers_end),
      ...
      (try_begin),
        ...
        (try_begin),
          (eq, ":continue", 1),
          (store_random_in_range, ":random_no", 0, 11),
          (try_begin),
            (eq, ":random_no", 0),
          ...
          (else_try),
            (eq, ":random_no", 3),#new add new bandit#libwe
            (assign, ":bandit_troop", "trp_mercenary_assassin"),
          (else_try),
            (eq, ":random_no", 4),
            (assign, ":bandit_troop", "trp_outlaw_knight"),
          (else_try),
            (eq, ":random_no", 5),
            (assign, ":bandit_troop", "trp_bloodlion_member"),
          (else_try),
            (eq, ":random_no", 6),
            (assign, ":bandit_troop", "trp_mafia4_warrior"),
          (else_try),
            (eq, ":random_no", 7),
            (assign, ":bandit_troop", "trp_black_khergit_horseman"),
          (else_try),
            (eq, ":random_no", 8),
            (assign, ":bandit_troop", "trp_shadow_warrior"),
          (else_try),
            (eq, ":random_no", 9),
            (assign, ":bandit_troop", "trp_slave_hunter"),
          (else_try),
            (assign, ":bandit_troop", "trp_brotherhood_marksman"),
          (try_end),
...
#new#Hunter_hut
  (14 * 24,
   [(try_for_range, ":cur_village", villages_begin, villages_end),
      (party_slot_eq, ":cur_village", slot_town_lord, "trp_player"),
      (party_slot_eq, ":cur_village", slot_center_has_hunter_hut, 1),
  ...
  [
    (try_for_range, ":cur_town", towns_begin, towns_end),
      ...
      (party_slot_eq, ":cur_town", slot_center_has_square, 1),
      (call_script, "script_change_center_prosperity", ":cur_town", 5),
      (party_get_slot, ":cur_relation", ":cur_town", slot_center_player_relation),
      (val_add, ":cur_relation", 1),
      (val_min, ":cur_relation", 100),
      (party_set_slot, ":cur_town", slot_center_player_relation, ":cur_relation"),
    (try_end),
  ]),
  #theater
  (15*24,
  [
    (try_for_range, ":cur_town", towns_begin, towns_end),
      (party_slot_eq, ":cur_town", slot_town_lord, "trp_player"),
      (party_slot_eq, ":cur_town", slot_center_has_theater, 1),
      (call_script, "script_change_center_prosperity", ":cur_town", 3),
    (try_end),
  ]),
  #Patrol station#=
  (10 * 24,
   [
    (call_script,"script_sprawn_patrol_party"),
    ]),
# Quest triggers:
...
[
    (try_for_parties, ":party_no"),
      ...
      (try_begin),
        ...
        (party_get_num_companions,":party_num",":party_no"),
        (call_script,"script_party_get_ideal_size",":party_no"),
        (assign,reg1,":party_num"),
        (party_set_extra_text, ":party_no", "@{s0}^{reg1}/{reg0}"),
      (try_end),
    (try_end),
]),
#############
